<div class='post-list-header'><%
    ...
    %><% if (ctx.canBulkDelete) { %><%
        %><form class='horizontal bulk-edit bulk-edit-delete'><%
            %><a href class='mousetrap button append open'>일괄 삭제</a><%
            %><input class='mousetrap start' type='submit' value='선택한 포스트 삭제'/><%
            %><a href class='mousetrap button append close'>삭제 멈추기</a><%
        %></form><%
    %><% } %><%
%></div>
